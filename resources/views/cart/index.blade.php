            ...
            @else
                ...
                <form method="post" action="/checkout">
                    ...
                    <div class="flex flex-col items-center justify-center gap-4 rounded-2xl border border-white/10 bg-white/5 p-6 md:flex-row md:items-center" data-cart-summary>
                        <input type="hidden" name="items" value="{{ json_encode(value: $items) }}">
                        <button class="rounded-full border border-white/20 px-4 py-2 text-[0.7rem] uppercase tracking-[0.2em] text-emerald-300 transition hover:border-emerald-400/60 hover:text-white" type="submit">Checkout</button>
                    </div>
                </form>
            @endif
        </div>
